@extends('layouts.app-master')

@section('title', 'Employees Management System | '.$employee->full_name)

@section('content')
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card mb-5" style="margin-top: -40px">
                    <div class="card-header bg-white text-center p-3">
                        <h3 class="text-dark">
                            Profile : {{$employee->full_name}}
                        </h3>
                    </div>
                    <div class="card-body">
                        <div class="form-group row mb-0">
                            <div class="col-md-12">
                                <a href="{{route('work.vacation',$employee->registration_number)}}"
                                   class="btn btn-outline-dark">
                                    {{ __('Vacation request') }}
                                </a>
                                <a href="{{route('work.certificate',$employee->registration_number)}}"
                                   class="btn btn-outline-danger">
                                    {{ __('Work certificate') }}
                                </a>
                            </div>
                        </div>
                        <hr>
                        <div class="form-group mb-3">
                            <label  class="form-label fw-bold">Full Name</label>
                            <div class="border border-secondary rounded p-2">
                                {{$employee->full_name}}
                            </div>
                        </div>
                        <div class="form-group mb-3">
                            <label  class="form-label fw-bold">Email</label>
                            <div class="border border-secondary rounded p-2">
                                {{$employee->email}}
                            </div>
                        </div>
                        <div class="form-group mb-3">
                            <label class="form-label fw-bold">Registration Number</label>
                            <div class="border border-secondary rounded p-2">
                                {{$employee->registration_number}}
                            </div>
                        </div>
                        <div class="form-group mb-3">
                            <label  class="form-label fw-bold">Departement</label>
                            <div class="border border-secondary rounded p-2">
                                {{$employee->depart}}
                            </div>
                        </div>
                        <div class="form-group mb-3">
                            <label  class="form-label fw-bold">Hired Since</label>
                            <div class="border border-secondary rounded p-2">
                                {{$employee->hire_date}}
                            </div>
                        </div>
                        <div class="form-group mb-3">
                            <label  class="form-label fw-bold">Phone</label>
                            <div class="border border-secondary rounded p-2">
                                {{$employee->phone}}
                            </div>
                        </div>
                        <div class="form-group mb-3">
                            <label class="form-label fw-bold">Address</label>
                            <div class="border border-secondary rounded p-2">
                                {{$employee->address}}
                            </div>
                        </div>
                        <div class="form-group mb-3">
                            <label  class="form-label fw-bold">City</label>
                            <div class="border border-secondary rounded p-2">
                                {{$employee->city}}
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection



